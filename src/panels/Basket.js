...
const Basket = ({ match: { params: { areaId, itemId }}, foodAreas, order }) => {
  const [ faster, setFaster ] = useState(localStorage.getItem('faster') === 'true' || false);
  const [ time, setTime ] = useState(localStorage.getItem('time') || '');
  const [ selfService, setSelfService ] = useState(localStorage.getItem('selfService') === 'true' || false);
  ...
  return (
    <div className="Place">
      ...
      <div className="Place__choice">
        ...
        <div className="Place__choice-item">
          <h3>С собой</h3>
          <Checkbox checked={selfService} onToggle={() => {localStorage.setItem('selfService', !selfService); setSelfService(!selfService)}} />
        </div>
        <div className="Place__choice-item">
          <h3>На месте</h3>
          <Checkbox checked={!selfService} onToggle={() => {localStorage.setItem('selfService', false); setSelfService(!setSelfService)}} />
        </div>
      </div>
